...
const CONNECTION_URL = "mongodb+srv://ntaevere:ntaevere123@coffee.nip9p.mongodb.net/coffiends?retryWrites=true&w=majority";
...
mongoose.connection.once('connected', async () => {
    ...
    const flatWhite = await Coffee.create({name: "Flat White", description: "coffee & milk"})
    const espresso = await Coffee.create({name: "Espresso", description: "single/double shot of coffee"})
    const capuccino = await Coffee.create({name: "Capuccino", description: "frothy milked coffee with a dash of chocolate sprinkles"})
    const latte = await Coffee.create({name: "Latte", description: "milky coffee"})
    const longBlack = await Coffee.create({name: "Long Black", description: "large black coffee"})
    const icedCoffee = await Coffee.create({name: "Iced Coffee", description: "like a latte but cold with ice n stuff"})
    ...
    const cafe02 = await Cafe.create(
        {
            name: "Cleberson’s Coffee . Cafe . Brisbane",
    ...
    const cafe16 = await Cafe.create(
        {
            name: "The Coffee Club Café - Brisbane Supreme Courts",
